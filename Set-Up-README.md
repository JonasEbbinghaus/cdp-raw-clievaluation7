...
## Overview
- **Product name:** `cdp_raw_clievaluation7`
- **Layer:** `raw`
- **Domain:** `Not provided`
...
The CLI that ran to create this repo performed the following actions for the given `cdp_raw_clievaluation7` and `raw`:
...
- Created an **Account level group** with read access for consumers: `cdp-raw-cdp_raw_clievaluation7-readers`. This group is intended to be the standard consumer group for granting read access to the product in UC.
...
- **`id`** *(Required)* — unique technical name of the data product (e.g., cdp_raw_clievaluation7). Was set automatically!
...
- **Access & Consumer Group** *(Required)* — group in the format cdp-raw-cdp_raw_clievaluation7-readers, description of how to request access.
...
- Group created: `cdp-raw-clievaluation7-readers` — add consumers here to grant read access to UC objects for this product.
...
- [ ] Confirm that `cdp-raw-clievaluation7-readers` group exists and members know how to request access
